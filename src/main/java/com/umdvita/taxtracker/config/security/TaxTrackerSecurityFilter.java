...
public class TaxTrackerSecurityFilter extends GenericFilterBean {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    Cookie rememberMe = WebUtils.getCookie(req, SecurityConfigConstant.REMEMBER_ME);
    if (Objects.nonNull(rememberMe)) {
      resp.addHeader(SecurityConfigConstant.SET_COOKIE, getSecuredCookie(rememberMe, request));
    }

    Cookie jsessionId = WebUtils.getCookie(req, SecurityConfigConstant.JSESSIONID);
    if (Objects.nonNull(jsessionId)) {
      resp.addHeader(SecurityConfigConstant.SET_COOKIE, getSecuredCookie(jsessionId, request));
    }
